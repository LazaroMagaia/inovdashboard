            ...
            <div class="w-full">
                <div class="bg-white shadow rounded-lg p-4">
                    <div class="flex justify-between items-center mb-4">
                        <h4 class="text-xl font-semibold text-[#849f54]">Gerencie os usuários</h4>
                        <button type="button" data-modal-target="#createUser" 
                            class="bg-[#849f54] text-white px-4 py-2 rounded-lg hover:bg-[#6b9677]">
                            Novo usuário
                        </button>
                    </div>
                    <div class="overflow-x-auto">
                        <table id="example" class="min-w-full bg-white border text-center">
                            <thead class="bg-[#849f54] text-white">
                                <tr>
                                    ...
                                    <th class="py-2 px-4 border-b text-xs font-medium uppercase tracking-wider">Nível de acesso</th>
                            ...
                            <tbody>
                                ...
                                <tr data-id="{{ $user->id }}">
                                    <td class="py-2 px-4 text-left border-b  text-sm text-gray-800 font-bold">
                                        {{$user->id}}
                                    </td>
                                    <td class="py-2 px-4 text-left border-b  text-sm font-medium text-gray-800">
                                        {{$user->first_name}} {{$user->last_name}}
                                    </td>
                                    <td class="py-2 px-4 text-left border-b  text-sm font-medium text-gray-800 font-bold">
                                        {{$user->hasRole('admin') ? 'Administrador' : 'Utilizador'}}
                                    </td>
                                    <td class="py-2 px-4 text-left border-b  text-sm font-medium text-gray-800 
                                        flex gap-2 justify-center">
                                        <button type="button" data-modal-target="#editUser{{$user->id}}" 
                                            data-role-id="{{$user->role_id}}" data-user-id="{{$user->id}}"
                                            class="bg-yellow-500 text-white px-4 py-2 rounded-lg
                                                 hover:bg-yellow-600">
                                            <i class="bi bi-pencil-square"></i>
                                        </button>
                                        
                                        <button type="button" data-modal-target="#deleteUser{{$user->id}}" 
...
@include('admin.modals.users.UsersModels')<script>
    $(document).ready(function() {
        let currentUserId; //id do user atual

        $('button[data-modal-target]').on('click', function() {
            const roleId = $(this).attr('data-role-id');
            currentUserId = $(this).attr('data-user-id');
            const group = $(`#grupoSingle${currentUserId}`);
            const fromStatus = $(`#from_status_div_edit${currentUserId}`);
            if (roleId == 2) {
                group.show();
                fromStatus.show();
            } else {
        ...
        // Adiciona o event listener para cada select dentro das modals
        $('select[id^="roles_select_edit"]').on('change', function() {
            const fromStatusDiv = $(`#from_status_div_edit${currentUserId}`);
            const group = $(`#grupoSingle${currentUserId}`);
